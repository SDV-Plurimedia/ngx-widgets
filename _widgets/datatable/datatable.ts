import {
  Component, ElementRef, IterableDiffers, Input, DoCheck, OnDestroy, OnInit, Output,
  EventEmitter
} from '@angular/core';
...
export class DatatableComponent implements DoCheck, OnDestroy, OnInit {
  ...
  public allVisibleRowsAreChecked() {
    if (this.table) {
      return jQuery(this._element.nativeElement).find('.row-checkbox:checked').length === this.table.page.len();
    } else {
      return false;
    }
  }
